...
* Open `superhero-logging-solved-no-functions.html` in `26-SuperheroLogging` in Chrome and in Sublime.
...
* Open `superhero-logging-solved-with-functions.html` in `26-SuperheroLogging` in Chrome and in Sublime.
...
* Open `joan-of-arc-arrays-unsolved.html` in `29-JoanOfArcArrays/Unsolved` in Sublime.
...
* Show how the relationship would be with `joan-of-arc-arrays-solved.html` in `29-JoanOfArcArrays/Solved`.
...
* Now open `gandalf-the-grey-objects.html` in `30-GandalfTheGreyObjects`.
  ...
  * Based on the current code, ask students: Why would we need to use bracket notation over dot notation?
    ...
    * Bracket notation is required when the property we want to access is a string with spaces. e.g. the code `object["multi-word property"]` would work, `object.multi-word property` or `object."multi-word property"` would not.
    ...
    * Had we just written `gandalf.ageProperty`, it would have returned `undefined`, since the `gandalf` object has no property named `ageProperty`.
...
* Open `car-game-solved.html` in `32-CarGame/Solution` in both the browser and in Sublime.
...
* Open and Slack out `scope-examples.html` in `09-Scope`.
...
* Open and Slack out `scope-unsolved.html` in `09-Scope/Unsolved`.
...
* Slack out `scope-solved.html` in `09-Scope/Solved` before moving on.
